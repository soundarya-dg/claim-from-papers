...
async def upload_batch_documents(
    ...
    Args:
        ...
        save_to_papers: Whether to save PDFs to the papers directory
        
    Returns:
        JSON response with batch ingestion results
    """
    results = {
        "status": "success",
    ...
    for file in files:
        try:
            # Process each file
            response = await upload_document(file, save_to_papers)
            results["successful"] += 1
            results["documents"].append(response.body.decode())
...
@router.post("/reset")
async def reset_vector_store() -> JSONResponse:
    """
